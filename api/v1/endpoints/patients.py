...
async def update_profile(
    ...
    if not pat:
        raise HTTPException(
            status_code=404,
            detail={"error": "Patient profile not found", "code": "NOT_FOUND"},
        )
...
async def get_appointments(
    ...
    appt_result = await db.execute(
        select(Appointment)
        .filter(Appointment.patient_id == pat.id)
...
async def get_record(
    ...
    if not doc:
        raise HTTPException(
            status_code=404, detail={"error": "Record not found", "code": "NOT_FOUND"}
        )
...
async def download_record(
    ...
    if not doc.file_url:
        raise HTTPException(
            status_code=404,
            detail={"error": "No file associated with this record", "code": "NO_FILE"},
        )
    ...
    if not os.path.exists(filepath):
        raise HTTPException(
            status_code=404,
            detail={"error": "File not found on server", "code": "FILE_NOT_FOUND"},
        )
